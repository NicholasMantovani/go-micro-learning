...
func (app *Config) authenticate(w http.ResponseWriter, a AuthPayload) {
	...
	jsonData, _ := json.MarshalIndent(a, "", "\t") // ONLY FOR DEV IN PROD USE json.Marshal

	// call the service
	request, err := http.NewRequest("POST", app.AuthBaseUrl+"/authenticate", bytes.NewBuffer(jsonData))
	if err != nil {
		app.errorJSON(w, err)
		return
	}
...
func (app *Config) sendMail(w http.ResponseWriter, msg MailPayload) {
	...
	request, err := http.NewRequest("POST", app.MailerBaseUrl+"/send", bytes.NewBuffer(jsonData))
	if err != nil {
		app.errorJSON(w, err)
	}
	request.Header.Set("Content-Type", "application/json")
	...
	if jsonFromService.Error {
		app.errorJSON(w, errors.New(fmt.Sprintf(jsonFromService.Message)))
		return
	}
